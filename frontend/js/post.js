...
const multi_input = document.getElementById("multi_input");
const multiInput = document.querySelector("multi-input");
const values = document.querySelector("#values");
const button = document.getElementById("send_post");
button.onclick = () => {
  if (multiInput.getValues().length > 0) {
    values.value = `${multiInput.getValues().join(" and ")}`;
  } else {
    values.value = "No Topic";
  }
...
/* 
 function UnlockSubmit() {
	...
	let valuesId = document.getElementById("topic_list");
	let contentId = document.getElementById("content").value;

	  console.log(titleId)
	  console.log(valuesId)
	  console.log(contentId)
	if (titleId == "", valuesId == "" ,contentId =="" ){
		document.getElementById("send_post").disabled = true;
